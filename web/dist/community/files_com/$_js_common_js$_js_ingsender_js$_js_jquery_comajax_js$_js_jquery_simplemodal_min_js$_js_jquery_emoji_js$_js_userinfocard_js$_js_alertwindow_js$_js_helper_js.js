var leagueJS = {
    myDatepicker: function (elmentID) {
        $("#" + elmentID).datepicker({
            dateFormat: 'yy-mm-dd',
        ...
        $o.mousemove(function (e) {
            $uploader.css({
                "left": e.pageX - $o.parents(".upload_form").offset().left - $uploader.width() + 10,
                "top": e.pageY - $o.parents(".upload_form").offset().top - $uploader.height() / 2
            });
        });
...
function showExpBOx(action, callback) {
    var options = {
        h3: ['发布主帖', '回复主贴', '签到'],
        h4: ['+2', '+1', '+1'],
        tips: [{text: '发表一篇有趣的帖子吧', num: '+2'}, {text: '给喜欢的帖子发个回复吧', num: '+1'}, {text: '完成今日签到', num: '+1'}]
    };
    ...
    var tipStr = '<div class="get_experience_box"><div class="info_box"><div class="rank_animate"></div><div class="top_tips"></div>' + '<h3>' + options.h3[action] + '</h3><h4>经验值 <span class="exp_num">' + options.h4[action] + '</span></h4><p class="tip_more">今日还可以赚取更多经验哦</p>';
...
function showQrBox(op, callback) {
    var _type = op.type, _bigType = op.bigType, _imgSrc = op.imgSrc ? op.imgSrc : "index_qr_code",
        _cookieName = op.cookieName ? op.cookieName : "hide_Qr_7day", qrStr = "",
        _types = {"0": "", "1": "想有评论", "2": "想有回复", "3": "想获得", "4": "立刻通知你", "5": "社刊更新提醒", "6": "订阅社刊成功"};
    var isReturn = true;
    if (typeof op.isSubscribe != "undefined") {
        qrStr = "<div class='wx-qr-box'><div class='wx-qr-content wx-qr-sub-content'>" + "<a href='http://st.hujiang.com/topic/164449981213/' target='_blank'><img src='/images/" + _imgSrc + ".png' width='160' height='160' /></a>" + "<p class='wx-qr-sub'><a href='javascript:;' class='btn btn-green go-look'>继续浏览</a><a href='/mymags/' class='btn btn-default check-my-sub'>查看我的社刊</a></p>" + "<div class='qr-sub-head-style'><p class='qr-head-title'><span class='big-font'>" + _types[_bigType] + "</span></p><p class='qr-sub-tips'>想获取社刊更新提醒吗</p><p class='qr-sub-scan'>微信扫一扫</p></div>" + "</div></div>";
    }
    else {
        if (League.fun.getCookie(_cookieName)) {
            callback && callback();
            isReturn = false;
        }
        qrStr = "<div class='wx-qr-box'><div class='wx-qr-content'>" + "<a href='http://st.hujiang.com/topic/164449981213/' target='_blank'><img src='/images/" + _imgSrc + ".png' width='160' height='160' /></a>" + "<p><a href='javascript:;' class='btn btn-m qr-btn'>知道了</a></p><p><a href='javascript:;' class='no-qr-tips'>不再提示</a></p>" + "<div class='qr-head-style'><p class='qr-head-title'>" + _types[_type] + "<span class='big-font'>" + _types[_bigType] + "</span>吗?</p><p class='qr-scan'>微信扫一扫</p></div>" + "</div></div>";
    }
    if (!isReturn) {
        return;
    }
    $('body').append(qrStr);
    $('body').on('click', '.no-qr-tips,.qr-btn,.go-look', function () {
        var self = $(this);
        if (self.hasClass('no-qr-tips')) {
            League.fun.setCookie(_cookieName, 1, 7 * 24 * 60 * 60, '/');
        }
        if (self.hasClass('qr-btn')) {
            League.fun.setCookie(_cookieName, 1, 24 * 60 * 60, '/');
        }
        if (self.hasClass('go-look')) {
            $('.wx-qr-box').remove();
        }
        $('.wx-qr-box').remove();
        callback && callback();
    ...
    ingSender.prototype = {
        init: function () {
            ...
            $(document).keyup(function (e) {
                ...
                if ((e.ctrlKey || e.metaKey) && e.keyCode == 120) {
            ...
            if (loginCookie == null || loginCookie == "") {
                $.ajax({
                    url: "http://t.hujiang.com/api/CheckLogin.ashx?callback=?",
                    dataType: "jsonp",
                    jsonp: "callback",
                    timeout: '1000',
                    async: false,
...
(function ($) {
    jQuery.extend({
        comajax: function (url, method, data, callback, errorCallback) {
            var para = data;
            if (para != null) {
                para = $.toJSON(data);
            }
...
(function ($) {
    ...
    $.modal.impl = {
        d: {}, init: function (data, options) {
            ...
            var oldHeight = $('#simplemodal-container .simplemodal-data').height(),
                oldWidth = $('#simplemodal-container .simplemodal-data').width();
            interval = setInterval(function () {
                var newHeight = $('#simplemodal-container .simplemodal-data').height(),
                    newWidth = $('#simplemodal-container .simplemodal-data').width();
            ...
            var mxoh = s.o.maxHeight ? s.getVal(s.o.maxHeight, 'h') : null,
                mxow = s.o.maxWidth ? s.getVal(s.o.maxWidth, 'w') : null, mh = mxoh && mxoh < w[0] ? mxoh : w[0],
            ...
            s.d.wrap.css({overflow: (dh > ch || dw > cw) ? 'hidden' : 'visible'});
            s.o.autoPosition && s.setPosition();
        }, setPosition: function () {
            var s = this, top, left, hc = (w[0] / 2) - (s.d.container.outerHeight(true) / 2),
...
(function () {
    var css_loading = "loading", css_userInfoCard = "userInfo-card", css_card_inner = "card-inner",
        css_head = "card-head", css_info = "card-info", css_buttons = "card-buttons", css_counters = "card-counters",
        css_st_infos = "card-st-infos", css_signature = "card-signature-info", css_title = "card-title-info",
        css_head_pic = "card_head-pic", baseApiPath = "http://bulo.hujiang.com",
        editNickNameUrl = baseApiPath + "/setting/normal/", Location = {};
    (function (L) {
        var cityCodes = {
            "110100": "北京",
            "120100": "天津",
    ...
    function follow(userId, userName, callback) {
        $.sync({
            url: '/v1/user/' + userId + '/addfollow/', type: 'post', userProxy: true, success: function (data) {
                callback(data);
            }
        });
    }
    ...
    function unfollow(userID, userName, callback) {
        if (window.confirm("确定要取消关注吗？")) {
            $.sync({
                url: '/v1/user/' + userID + '/removefollow/',
                type: 'post',
                userProxy: true,
                success: function (data) {
                    callback(data);
    ...
    function getRelation(code) {
        return {
            "-3": relations.anonymous,
            "-2": relations.none,
            "-1": relations.blackList,
            "0": relations.focused,
            "1": relations.friend,
            "2": relations.self,
            "3": relations.fans
    ...
    var template = "<div id='userInfoCard' class='" + css_userInfoCard + "'>" + "<div class='" + css_loading + "'></div>" + "<div class='" + css_card_inner + "'>" + "<div class='top'>" + "<a href='/mall/daoju/' target='_blank' title='更多酷炫道具，快去商城兑换！' class='decoration-icon'></a>" + "<div class='" + css_head + "'></div>" + "<div class='" + css_info + "'></div>" + "<div class='" + css_buttons + "'></div>" + "</div>" + "<div class='" + css_counters + "'></div>" + "<div class='" + css_st_infos + "'></div>" + "<hr />" + "<div class='" + css_signature + "'></div>" + "<div class='hide " + css_title + "'>" + "</div>" + "</div>";
    UserInfoCard.prototype = {
        constructor: UserInfoCard, init: function () {
            this._initElements();
            this._initEvents();
        }, _initElements: function () {
            var appendTo = this.options.appendTo || "body",
                container = this.container = $(template).appendTo(appendTo).hide(), self = this, innner;
        ...
        }, _titleCardDec: function (data) {
            $('#userInfoCard').removeClass().addClass(css_userInfoCard);
            if (data.userCardDecorate != '' && data.userCardDecorate != null) {
                $('.' + css_userInfoCard).addClass('userInfo-card-dec ' + data.userCardDecorate);
            }
            ...
            if (relation != relations.self) {
                ...
                if (relation != relations.anonymous) {
                    html += format("<a class='ccTalk icon'><iframe src='http://service.cctalk.com/api/frame_install/frame_cctalkInstall.aspx?userid={0}&userName={1}' scrolling='no' frameborder='no' allowtransparency='true'></iframe></a>", data.userID, data.userName);
                }
            }
            ...
            if (data.leagues && data.leagues.length) {
                ...
                this.$stInfos.html("<div class='empty-st'>暂无加入学社</div>");
            }
            ...
            var target = this.options.target, o = this.options, self = this, showTimer, hideTimer, delay = 500,
                isMouseInside;
            ...
            self.userId = userId;
            $.sync({
                url: '/v1/user/' + userId + '/card/', type: 'get', userProxy: true, success: function (data) {
                    self._switchLoadingState(false);
                    self._initView(data);
                }
            });
...
$(function () {
    new UserInfoCard({
        target: "a[href*='http://bulo.hujiang.com/'],a[href*='http://t.hujiang.com/'],a[href*='/listen/u'],.userface,a[href*='/u/']",
        getUserId: function (ele) {
            var nouserface = ele.attr("nouserface");
            if (!nouserface) {
                var href = ele.attr("href");
                var userID = ele.attr("userid") || (href.match(/\d+/) && href.match(/\d+/)[0]);
                var host = href && href.replace(/(\/u)?\/\d+\/?/ig, "");
                if (userID && (host == "http://bulo.hujiang.com" || host == "bulo.hujiang.com" || host == "t.hujiang.com" || host == "http://t.hujiang.com")) {
                    return userID;
                }
                if (userID && location.host.indexOf('st.hujiang.com') >= 0 && location.pathname.indexOf('team') > 0) {
                    return userID;
                }
            }
            return null;
...
(function (win) {
    ...
    function confirmWindow(title, info, callback) {
        ...
        infoBox += '<a href="javascript:;" class="confirm_Ok_btn">确认</a>' + '<a href="javascript:$.modal.close();" class="confirm_close_btn">取消</a></div>';
        infoBox = $(infoBox);
        infoBox.modal();
        infoBox.on('click', '.confirm_Ok_btn', function () {
...
(function ($, win) {
    $.extend($, {
        stringify: function (obj) {
            ...
            return s.object(obj);
        }, videoFormate: function ($target) {
            var UA = navigator.userAgent, reg = /ipad|android|KFAPWI/i;
            if (reg.test(UA)) {
                $target.find('embed').each(function (k, t) {
                    var $t = $(t), _width = $t.width(), _height = $t.height();
                    hjVideoPlayer({source: $t.attr('src'), width: _width, height: _height}).then(function (data) {
                        var _videoElement = $(data.result).width(_width).height(_height);
                        $t.replaceWith(_videoElement);
                    });
                })
            }
            ...
            var defaultSetting = {
                    ...
                    error: null
                }, opt = $.extend(true, defaultSetting, options), isRetina = $.isRetina(),
                windowHeight = isRetina ? window.devicePixelRatio * document.documentElement.clientHeight : $(window).height(),
                loading = false, loadingObj = $(opt.loading), uniqID = $.uniqID(), curPage = opt.data.page;

            function loadData() {
                opt.data.page = curPage;
                var data = {
                    url: opt.url, type: 'get', data: opt.data, dataType: opt.dataType, success: function (d) {
                        var hasMore = true;
            ...
            var img = "";
            $.ajax({
                url: '/ajax/optionAjax.ashx?action=getuseravatar&userId=' + userId,
                dataType: "json",
                async: false,
                success: function (d) {
                    if (d.ret == 0) {
            ...
            function getDateStr(d) {
                return d.replace('T', ' ').replace(/-/g, '/').split('+')[0].split('.')[0];
            }

            date = getDateStr(date);
            var ts = new Date(getDateStr($('#hddTime').val())).getTime() - new Date(date).getTime();
            if (!date || ts <= 0) return '刚刚';
            ...
            return date.substr(0, date.length - 3).replace(/\//g, '-');
        }, formatDateNew: function (date, fmt) {
            if (!date) return '';
        ...
        }, pageFormat: function (currentPage, pageCount, orderType) {
            var strNav = "";
            var divPageNum = 0;
            if (pageCount < 2) {
                return strNav;
            ...
            if (currentPage > 1) {
                strNav += "<li class=\"page-l\"><a href='javascript:;' data-page='" + (currentPage - 1) + "' data-order='" + orderType + "'>上一页</a></li>";
            }
            if (pageCount == 0) {
                strNav += "<li>1</li>";
            }
            if (currentPage > 6) {
                strNav += "<li><a href='javascript:;'  data-page='1' data-order='" + orderType + "' class='otherpage'>1</a></li><span>... </span>";
            }
            var leftPageCount = 5;
            ...
            if (pageCount > currentPage + leftPageCount) {
                divPageNum = currentPage + leftPageCount;
            }
            else {
                divPageNum = pageCount;
            }
            for (var i = currentPage - leftPageCount - 1; i < divPageNum; i++) {
                if (i >= 0) {
                    if (currentPage == i + 1) {
                        strNav += "<li class=\"active\">" + (i + 1) + "</li>";
                    }
                    else {
                        strNav += "<li><a href='javascript:;' data-page='" + (i + 1) + "' data-order='" + orderType + "'>" + (i + 1) + "</a></li> ";
                    }
                }
            }
            if (currentPage + leftPageCount < pageCount) {
                if (currentPage + leftPageCount + 1 < pageCount) {
                    strNav += "<span>... </span>";
                }
                strNav += "<li><a href='javascript:;' data-page='" + pageCount + "' data-order='" + orderType + "' class='otherpage'>" + pageCount + "</a></li> ";
            }
            if (currentPage != pageCount) {
                strNav += "<li class='page-l'><a href='javascript:;' data-page='" + (currentPage + 1) + "' data-order='" + orderType + "' class='prevNext'>下一页</a></li>";
            }
            return strNav;
        }, checkCtIsOnline: function (userId, callback) {
        }
    });
    ...
    $.extend($, {
        sync: function (options) {
            var defaultSetting = {checkNetwork: true, timeout: 40000, userProxy: true};
